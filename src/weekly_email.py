...
Pipeline:
  ...
  6. Deliver it (unless --dry-run or delivery disabled)

Email delivery goes through src/_delivery (Resend today).
"""
...
def deliver_digest(label: str, md: str, partial: bool) -> None:
    """Send the assembled digest through the configured delivery backend.

    Email rendering + HTTP lives in src/_delivery (Resend today). The
    raw `md` here includes voice stats and auditor sections; we reuse the
    viewer's `assemble_week_markdown` so the email matches the viewer's
    clean themes + problems layout.
    """
    from _delivery.render import week_short_from_label
    prefix = "[partial] " if partial else ""
    subject = f"{prefix}Weekly Life Recap ({week_short_from_label(label)})"

    # Use the viewer-assembled body, not the raw digest, so the email skips
    # voice stats and the auditor's "What I noticed" section.
    try:
        import build_viewer
        body_md = build_viewer.assemble_week_markdown(label)
    except Exception:
        body_md = md  # graceful fallback

    import _delivery  # local import; _delivery is optional infra
    _delivery.send_digest(subject, body_md, label)
    log(f"  delivered: {subject!r}")
...
def main() -> int:
    ...
    try:
        ...
        try:
            deliver_digest(target, digest_md, partial)
